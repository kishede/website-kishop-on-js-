document.addEventListener('DOMContentLoaded', function () {
	...
	function updateCartCounter() {
		...
		if (sidebarCartCounter) {
			sidebarCartCounter.textContent = total
		}
		...
		if (sidebarCartCounter) {
			sidebarCartIcon.classList.add('added')
			setTimeout(() => sidebarCartIcon.classList.remove('added'), 500)
		}
		updateSidebarCartCounter()
	}
	...
	document.querySelectorAll('.buy-button').forEach(button => {
		button.addEventListener('click', function () {
			...
			alert(
				`${productName} добавлен в корзину! (Всего: ${
					cart[existingItemIndex]?.quantity || 1
				})`
			)
		})
